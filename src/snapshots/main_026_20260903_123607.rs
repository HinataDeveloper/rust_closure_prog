...
fn main() {
    ...
    let number_list: Vec<u32> = vec![1, 2, 3, 4, 5, 6, 7, 8, 9];

    let map_collect: Vec<u32> = number_list.iter().map(|data| data + 10).collect();

    let resultant: u32 = map_collect.iter().sum();
    println!("value of resultant is: {}", resultant);
    println!("~~~~~~~~~~~~~~~~");

    map_collect.iter().for_each(|data| {
        println!("{}", data);
    });
