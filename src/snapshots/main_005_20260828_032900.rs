...
// Goal: Creating function inside a function
...
fn main() {
    ...
    let result = add_v1(120);
    println!("value of result is: {}", result);

    // creating a function inside a function in two level
    fn add_v2(mut x: u32) -> u32 {
        fn add_inside_1(x: u32) -> u32 {
            x + 1
        }
        x += 1;
        add_inside_1(x)
    ...
    let result_two = add_v2(330);
    println!("value of result two is: {}", result_two);
