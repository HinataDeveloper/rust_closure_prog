...
fn main() {
    ...
    let v1: Vec<i32> = number_list.iter().map(|d| d + 10).collect();

    assert_eq!(v1, vec![11, 12, 13, 14, 15, 16, 17, 18, 19]);
    println!("assertion check was passed ...");

    number_list.iter().for_each(|d| {
        println!("{}", d);
    });
